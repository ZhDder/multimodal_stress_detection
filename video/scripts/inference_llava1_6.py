...
class StressWindowDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Load and uniformly resize all frames to LLaVA's native base resolution
        images = []
        for p in image_paths:
            img = Image.open(p).convert("RGB")
            img = img.resize((336, 336)) # 336x336, llavas native resolution
            images.append(img)
...
def build_messages():
    return [
        ...
        {"role": "user", "content": [
            {"type": "video"}, # Replace the list of 'image' types with a single 'video' type
...
if __name__ == '__main__':
    ...
    with torch.inference_mode():
        with tqdm(loader, desc="Inferring", unit="window",
            ...
            for batch in pbar:
                ...
                #Render the text prompts (inserts the <video> tokens automatically)
                texts = processor.apply_chat_template(
                    batch_messages,
                    tokenize=False, # Just return the strings
                    add_generation_prompt=True
                )

                #Pass the text and your lists of frames directly to the processor
                # batch["images"] is a list of lists of PIL images, which perfectly matches
                # the expected input for batched videos
                inputs = processor(
                    text=texts,
                    videos=batch["images"], 
                    padding=True,
                ...
                # Decode (using the standard slicing technique)
    ...
    out_path = "/workspace/Gemma3_4B_Instruct_cropped_results.csv"
